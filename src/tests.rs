...
fn test_read_write_encrypt() {
    ...
    let mut data = pk.encrypt(&mut OsRng, PaddingScheme::PKCS1v15Encrypt, data.as_slice()).unwrap();
    ...
    for chunk in de_data.chunks(pk.size() - 11) {
        let mut d = pk.encrypt(&mut OsRng, PaddingScheme::PKCS1v15Encrypt, chunk).unwrap();
